...
async function bootstrap() {
  ...
  app.use(mung.json(
    function transform(body, req, res) {
      ...
      if (  [`${AppModule.globalPrefix}/auth/login`, `${AppModule.globalPrefix}/auth/token/refresh`].includes(req.url) && contype && contype.indexOf('application/json') === -1 ) {

        const accessCookieCfg: any = {
          maxAge: AppModule.jwtAccessTokenExpirationTime * 1000,
          path: AppModule.globalPrefix,
        };

        const refreshCookieCfg: any = {
          maxAge: AppModule.jwtRefreshTokenExpirationTime * 1000,
          path: `${AppModule.globalPrefix}/auth/token/refresh`,
        };
