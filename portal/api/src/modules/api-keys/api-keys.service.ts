import { Injectable, NotAcceptableException, UnauthorizedException, Logger, HttpException, NotFoundException } from '@nestjs/common';
...
export class ApiKeysService {
  ...
  async redeem(owner: string, apiKeyId: string): Promise<ResponseApiKeyDto> {
    const apiKey = await this.findOne(owner, apiKeyId);
    if (apiKey) {
      try {
        const ledgerApiKeyData = await this.apiKeysContract.methods.getApiKey(apiKey.name).call();
        const dateLimit = new Date((Number(ledgerApiKeyData.startDate) + Number(ledgerApiKeyData.time)) * 1000);
        await this.apiKeysRepository.updateOne({_id: new ObjectId(apiKeyId), owner}, {$set: {expirationDate: dateLimit}}, { upsert: true});
        return await this.findOne(owner, apiKeyId);
      } catch (err) {
        this.log.debug(err);
