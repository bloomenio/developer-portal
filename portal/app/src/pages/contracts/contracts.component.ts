...
import { map, filter, debounce, distinctUntilChanged, tap } from 'rxjs/operators';
...
export class ContractsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public ngOnInit() {
    this.displayedColumns = ['id', 'name', 'address', 'public', 'actions'];
  ...
  public deleteContract(item) {
    const dialogRef = this.dialog.open(GeneralDialogComponent, {
      ...
      data: {
        title: `${item.name}`,
        description: this.translateService.instant('contracts.delete_message'),
